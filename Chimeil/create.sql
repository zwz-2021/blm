...
#NSERT INTO `dish` VALUES (67,'鮰鱼2斤',16,72.00,'https://sky-itcast.oss-cn-beijing.aliyuncs.com/8cfcc576-4b66-4a09-ac68-ad5b273c2590.png','原料：鮰鱼，黄豆芽，莲藕',1,'2022-06-10 10:43:56','2022-06-10 10:43:56',1,1);
...
CREATE TABLE `shopping_cart` (
  ...
  `amount` decimal(10,2) NOT NULL COMMENT '金额',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `cart_dish_id` bigint GENERATED ALWAYS AS (ifnull(`dish_id`,0)) STORED COMMENT 'normalized dish id for unique cart item',
  `cart_setmeal_id` bigint GENERATED ALWAYS AS (ifnull(`setmeal_id`,0)) STORED COMMENT 'normalized setmeal id for unique cart item',
  `cart_dish_flavor` varchar(50) COLLATE utf8_bin GENERATED ALWAYS AS (ifnull(`dish_flavor`,'')) STORED COMMENT 'normalized flavor for unique cart item',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_shopping_cart_user_item_flavor` (`user_id`,`cart_dish_id`,`cart_setmeal_id`,`cart_dish_flavor`)
